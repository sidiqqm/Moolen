...
import { loginWithGoogleAndBackend, registerUserWithBackend } from '../lib/auth';
...
const RegisterPage = () => {
  ...
  const handleGoogleRegister = async () => {
    setLoading(true);
    setError('');
    const result = await loginWithGoogleAndBackend(); // Panggil fungsi baru
    setLoading(false);
    if (result.success) {
      alert(result.message || 'Login dengan Google berhasil! Mengarahkan ke halaman utama...');
      navigate('/');
    } else {
  ...
  const handleSubmit = async (e) => { // Jadikan async
    ...
    const result = await registerUserWithBackend(email, password, firstName);
    setLoading(false);
    if (result.success) {
    ...
    <div className="min-h-screen flex items-center justify-center mt-12">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        ...
        <div className="space-y-3">
          <button
            onClick={handleGoogleRegister} // Panggil handler baru
          ...
          {/* Tombol Apple bisa diintegrasikan dengan Firebase juga jika diperlukan */}
          <button className="w-full flex items-center justify-center gap-2 bg-black text-white rounded-full py-3 shadow-md hover:opacity-90 transition disabled:opacity-50" disabled={loading}>
